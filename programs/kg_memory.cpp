...
int main() {
    ...
    SDL_Window* window = SDL_CreateWindow("KernelGuard — Memory Detect",
    ...
    while (running) {
        ...
        ImGui::BeginChild("##secret", {0, 90.f}, true);
        {
            ...
            ImVec4 val_col = intruder ? COL_THREAT : COL_OK;

            ImGui::TextColored(COL_DIM, "Name    "); ImGui::SameLine();
            ImGui::TextColored(val_col, "secret_value");

            ImGui::TextColored(COL_DIM, "Value   "); ImGui::SameLine();
            ImGui::TextColored(val_col, "%s  (%d)", dec, (int)secret_value);
