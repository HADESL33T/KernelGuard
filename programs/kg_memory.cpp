// kg_memory.cpp — KernelGuard: soft-detect via /proc/self/status TracerPid
// Polls TracerPid every 500 ms using ImGui frame timing (no sleep_for).
...
// Do NOT change — reads /proc/self/status for TracerPid
...
int main() {
    ...
    volatile int secret_value = 0xCAFEBABE;
    ...
    log.push_back({now_str(), "Monitoring started — polling /proc/self/status", COL_INFO});
    ...
    while (running) {
        ...
        // Poll TracerPid every 500 ms via frame timing
        ...
        // Title
        ImGui::TextColored(COL_AMBER, "KernelGuard v1.0");
        ImGui::SameLine();
        ImGui::TextColored(COL_DIM, "— Memory Detect");
        ImGui::Separator();

        // Badge
        if (intruder)
            ImGui::TextColored(COL_THREAT,
                "[!!!] INTRUDER DETECTED — TracerPid = %d", cur_tracer);
        else
            ImGui::TextColored(COL_OK, "[OK] Safe — no tracer attached");
        ImGui::Separator();
        ...
        char pid_str[32], tracer_str[32], up_str[32];
